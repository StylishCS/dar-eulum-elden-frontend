...
export function TransactionsTable({ merchants }) {
  ...
  const handleSearchChange = (e) => {
    ...
    filterMerchants(e.target.value);
  };

  const filterMerchants = (searchTerm) => {
    const filtered = allMerchants.filter(
      (merchant) =>
        merchant.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        (merchant.UID &&
          merchant.UID.toLowerCase().includes(searchTerm.toLowerCase()))
    );
    ...
    if (currentClient) {
      try {
        ...
        const res = await http.PATCH(
          `http://localhost:3000/dashboard/merchants/${currentClient._id}`,
          { amount: paidAmount }
        );
        setAllMerchants((prevMerchants) =>
          prevMerchants.map((merchant) =>
            merchant._id === currentClient._id
              ? { ...merchant, due: newDueAmount }
              : merchant
          )
        );
          ...
          <table className="w-full min-w-max table-auto text-left">
            ...
            <tbody>
              {currentMerchants.map(
                ({ _id, name, UID, phone, due, updatedAt }) => {
