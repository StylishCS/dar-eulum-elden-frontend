  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/dashboard/merchants`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <>
      ...
      <Breadcrumbs className="mb-4">
        ...
        <Link to="/dashboard/merchants/add">New Merchant</Link>
      </Breadcrumbs>
      <Dialog open={open} handler={handleOpen}>
        <DialogHeader>Merchant Profile Created Successfully</DialogHeader>
        <DialogBody>
          <h1>Merchant ID: {newClient.UID}</h1>
          <h1>Merchant Name: {newClient.name}</h1>
          <h1>Merchant Phone: {newClient.phone}</h1>
        </DialogBody>
        <DialogFooter>
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Button
              ...
              disabled={loading}
            >
              {loading ? <Spinner /> : "Create Merchant"}
            </Button>
          </div>
        </div>
      </div>
